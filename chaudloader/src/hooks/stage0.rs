...
use crate::{assets::{self, audio::create_chaudpck, WriteSeek}, config, gui, mods::{self, ModAudioFiles, ModFunctions, MODAUDIOFILES, MODFUNCTIONS}};
    ...
    // We are done with mod initialization! We can now go repack everything from our overlays.
    {
      ...
      
        {
            ...
            if !mod_audio.wems.is_empty() {
                ...
                assets_replacer.add(Path::new("..\\\\exe\\\\audio\\\\chaudloader.pck"),move |writer|  create_chaudpck(writer ) );
...

        {
            ...
            for (modpath,tonewpath) in mod_audio.modPaths.clone(){
                
                assets_replacer.add(&tonewpath, move |writer| {
                    
                    Ok(modpath.clone().create_otherpck(writer)?)
                });
            }
